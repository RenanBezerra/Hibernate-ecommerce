 ...
 * org.hibernate.engine.config.spi.ConfigurationService; import
 ...
 * SQLException { Connection connection = getAnyConnection();
 * 
 * try { connection.createStatement().execute("use " + tenantIdentifier); }
 * catch (SQLException e) { throw new
 * HibernateException("Nao foi possivel alterar " + "para o schema " +
 * tenantIdentifier + ". ", e);
 * 
 * }
 * 
 ...
 * SQLException { connectionProvider.closeConnection(connection); }
 * 
 * @Override public boolean supportsAggressiveRelease() {
 * 
 * return connectionProvider.supportsAggressiveRelease(); }
 * 
 * @Override public boolean isUnwrappableAs(Class unwrapType) { return
 * connectionProvider.isUnwrappableAs(unwrapType); }
 * 
 * @Override public <T> T unwrap(Class<T> unwrapType) { return
 * connectionProvider.unwrap(unwrapType); }
 * 
 * @Override public void injectServices(ServiceRegistryImplementor
 * serviceRegistry) { this.properties =
 * serviceRegistry.getService(ConfigurationService.class).getSettings(); }
 * 
 * @Override public void start() { HikariCPConnectionProvider cp = new
 * HikariCPConnectionProvider(); cp.configure(this.properties);
 * 
