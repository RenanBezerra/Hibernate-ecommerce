...
public class LockPessimistaTest {
	...
	public void usarLockNaTypedQuery() {
		Runnable runnable1 = () -> {
			...
			String novaDescricao = "Descricao detalhada. CTM: " + System.currentTimeMillis();

			EntityManager entityManager1 = entityManagerFactory.createEntityManager();
			entityManager1.getTransaction().begin();

			log("Runnable 01 vai carregar todos os produtos.");
			List<Produto> lista = entityManager1.createQuery("select p from Produto p where p.id in (3,4,5)")
					.setLockMode(LockModeType.PESSIMISTIC_READ).getResultList();

			Produto produto = lista.stream().filter(p -> p.getId().equals(3)).findFirst().get();
			...
			log("Runnable 01 vai confirmar a transacao.");
		...
		Runnable runnable2 = () -> {
			...
			String novaDescricao = "Descricao massa! CTM: " + System.currentTimeMillis();
			...
			log("Runnable 02 vai confirmar a transacao");
		...
		Assert.assertTrue(produto.getDescricao().startsWith("Descricao massa!"));
	...
	public void misturarTiposDeLocks() {
		...
		Assert.assertTrue(produto.getDescricao().startsWith("Descricao detalhada"));
